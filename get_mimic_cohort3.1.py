...
def main(args):
    ...
    MIMIC_31_DIR = args.raw_data_dir

    df_admission = pd.read_csv(os.path.join(MIMIC_31_DIR, "admissions.csv.gz"))
    ...
    ### Get demographics
    df_patients = pd.read_csv(os.path.join(MIMIC_31_DIR, "patients.csv.gz"))
    ...
    for _, row in tqdm(
        ...
        race = df_admission[df_admission["subject_id"] == subject_id][
            "race"
        ].values[0]
    ...
    ### Medication
    df_presb = pd.read_csv(os.path.join(MIMIC_31_DIR, "prescriptions.csv.gz"))
    df_presb.head()
    ...
    ### ICD-10
    df_diag = pd.read_csv(os.path.join(MIMIC_31_DIR, "diagnoses_icd.csv.gz"))

    df_diag_table = pd.read_csv(os.path.join(MIMIC_31_DIR, "d_icd_procedures.csv.gz"))
    ...
    ### Get labs
    df_lab_item = pd.read_csv(os.path.join(MIMIC_31_DIR, "d_labitems.csv.gz"))
    lab_item_idxs = []
    ...
    df_lab_filtered = []
    with pd.read_csv(
        os.path.join(MIMIC_31_DIR, "labevents.csv.gz"), chunksize=1000
    ) as reader:
        for chunk in reader:
            chunk = chunk[
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--raw_data_dir", type=str, help="Dir to downloaded MIMIC-IV data.", default="/home/mjkim1/workspace/readmit-stgnn/3.1_csv"
    )
    parser.add_argument(
        "--cxr_data_dir", type=str, help="Dir to downloaded MIMIC-CXR-JPG data.", default="/home/mjkim1/workspace/readmit-stgnn/cxr_csv"
    )
    parser.add_argument(
        "--save_dir", type=str, help="Dir to save filtered cohort files.", default="./"
    )
